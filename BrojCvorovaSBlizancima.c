...
int broj_blizanaca(Node *node) {
    ...
    int blizanci = 0;
    int left = broj_blizanaca(node->left);
    int right = broj_blizanaca(node->right);

    blizanci = (node->left && node->right && node->left->key == node->right->key) ? 1 : 0;

    return left + right + blizanci;
}
