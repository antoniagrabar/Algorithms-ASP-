...
void najudaljeniji_vrhovi(graph *g, vertex *u) {
    int queue[g->nvertices];
    vertex *v, *najudaljeniji[g->nvertices];
    int top = -1, head = -1, tail = -1, max_dist = 0;
    edge *e;

    initialize_attributes(g);
    queue[++tail] = find_vertex_index(g,u);
    u->color = GRAY;
    u->distance = 0;

    while(head != tail){
        u = g->vertices[queue[++head]];
        for(e = u->edges; e; e = e->next){
            v = e->v;
            if(v->color == WHITE){
                v->color = GRAY;
                v->distance = u->distance + 1;
                queue[++tail] = find_vertex_index(g,v);
            }
        }
        u->color = BLACK;
    }
    ...
    for (int i = 0; i <= top; i++){
        printf("%s ", (najudaljeniji[i])->label);
    }
...
int main() {
    ...
    setbuf(stdout, NULL);
    do {
        DEBUG("\n1.Najudaljeniji vrhovi (najudaljeniji_vrhovi)");
        DEBUG("\n2.Unos grafa\n3.Ispis grafa\n4.Izlaz\n");
        scanf("%d", &menu_choice);
