...
void najudaljeniji_vrhovi(graph *g, int u) {
    int queue[MAXV], tail = -1, head = -1;
    int najudaljeniji[MAXV], top = -1, max_dist = 0;
    ...
    //u najudaljeniji se spremaju indeksi vrhova koji imaju istu max udaljenost
    for (int v = 0; v < g->nvertices; v++){
        if(g->distances[v] == max_dist){
            najudaljeniji[++top] = v;
        } else if(g->distances[v] > max_dist){
            max_dist = g->distances[v];
            top = -1;
            najudaljeniji[++top] = v;
        }
    }
    ...
    for (int v = 0; v <= top; v++){
        printf("%c ", g->labels[najudaljeniji[v]]);
    }
    printf("\n%d\n", max_dist);
