...
//broj upisanih cvorova
int node_num(Node *node){
    if(!node) return 0;

    return node_num(node->left) + node_num(node->right) + 1;
}
...
int max_height(Node *node){
    ...
    left = max_height(node->left) + 1;
    right = max_height(node->right) + 1;

    return left > right ? left : right;
}

//max broj cvorova u visini h je 2^(h+1)-1
//broj cvorova koje je moguce upisati je max broj cvorova - broj upisanih cvorova node_num

void broj_cvorova(Node *root) {
    int max = pow(2, (max_height(root)+1) ) - 1;
    printf("%d\n", root ? max - node_num(root) : 0);
}
