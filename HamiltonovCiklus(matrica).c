...
bool hamiltonov_ciklus(graph *g, int u, int distance) {

    if(distance == g->nvertices-1 && distance > 1){
        for(int v = 0; v < g->nvertices; ++v){
            if(g->adj[u][v] && g->distances[v] == 0){
                return TRUE;
            }
        }
    }
    g->colors[u] = GRAY;
    g->distances[u] = distance;
    for(int v = 0; v < g->nvertices; ++v){
        if(g->adj[u][v]){
            if(g->colors[v] == WHITE){
                if(hamiltonov_ciklus(g, v, distance+1)){
                    return TRUE;
                }
            }
        }
...
int main() {
    ...
    setbuf(stdout, NULL);
    do {
        DEBUG("\n1.Postoji li Hamiltonov ciklus (hamiltonov_ciklus)");
        DEBUG("\n2.Unos grafa\n3.Ispis grafa\n4.Izlaz\n");
        scanf("%d", &menu_choice);
        switch(menu_choice) {
            case 1:
                initialize_attributes(g);
                printf("%c\n", hamiltonov_ciklus(g, 0, 0) ? 'Y' : 'N');
                break;
            case 2:
                if (g) free(g);
                g = initialize_graph();
